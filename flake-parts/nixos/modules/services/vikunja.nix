# FIXME: this is pulled from 24.05
{ pkgs, lib, config, ... }:
...
in {
  imports = [
    ...
    # when upgrading, vikunja should be patched to support service user
    {
      base.vikunja.package = pkgs.unstable.vikunja;

      systemd.services.vikunja.serviceConfig = lib.mkForce {
        ExecStart = "${config.base.vikunja.package}/bin/vikunja";
        Restart = "always";
        User = "vikunja";
        WorkingDirectory = "/var/lib/vikunja";
        EnvironmentFile = config.base.vikunja.environmentFiles;
      };
